...
const { removeCountryFromLanguage, translate } = require('./deeplTranslator');
...
const storage = new Storage({ projectId: "app-mobile-translation-chat" });
...
exports.sendMessageNotification = functions.database.ref('/messages/{messageId}').onCreate((snap, context) => {
  ...
  return Promise.all([ getSenderInfo, isUserBlocked]).then(results => {
    ...
    const refStr = type === 1 ? 'userMessagesNotTranslated' : 'userMessages'
    
    //index messages
    return admin.database().ref(`${refStr}/${toId}/${messageId}`).set(message.data).then(() => {
...
exports.translateTextMessages = functions.database.ref(`userMessagesNotTranslated/{uid}/{messageId}`).onCreate(async(snapshot,context) =>{
  const uid = context.params.uid
  const messageId = context.params.messageId
  const user = await (await admin.database().ref(`users/${uid}`).once('value')).val()
  const dstLang = user.lang
  const message = snapshot.val()
  

  if(message.lang !== dstLang){
    const translatedText = await translate(message.content, getLangWithNoAccent(dstLang))
  ...
  await admin.database().ref(`userMessages/${uid}/${messageId}`).set(message)
  await admin.database().ref(`userMessagesNotTranslated/${uid}/${messageId}`).remove()
  const fcmMessage = getMessagePayload(message.type, message, message.fromPhone, message.content, message.timestamp, message.fromId, message.toId, message.isGroup, messageId, message.metadata);
  fcmMessage.data = removeExtraLimitForPayload(message.data)
  fcmMessage.tokens = Object.keys(user.notificationTokens)

  return messageSender.sendMulticastMessage(message, uid)

})

exports.transcribe = functions.https.onCall(async (_data, context) => {
  if (!context.auth) {
  ...
  const messageId = _data.messageId
  const audioLang = _data.lang
  const userLang = _data.dstLang
  if (!content || !messageId || !audioLang || !userLang) {
    throw new functions.https.HttpsError('invalid request', 'content not availalbe');
  }

  const cachedTranscription = await admin.database().ref(`cached_transcribed/${messageId}`).once('value')

  if(cachedTranscription.exists()){
    const transcribedText = cachedTranscription.val()
    if(audioLang === userLang){
      return transcribedText
    }
    return translate(transcribedText, getLangWithNoAccent(userLang))
  }

  const transcribedText = await transcribeAudio(content, audioLang)
  await admin.database().ref(`cached_transcribed/${messageId}`).set(transcribedText)
  if(audioLang === userLang){
    return transcribedText
  }
  return translate(transcribedText, getLangWithNoAccent(userLang))
...
exports.sendMessagesForGroups = functions.database.ref(`/groupsMessages/{groupId}/{messageId}`).onCreate((snap, context) => {
  ...
  return admin.database().ref(`groups/${groupId}/users`).once('value').then((usersSnap) => {
    ...
    users.forEach(uid => {
      ...
      if (uid !== fromId) {
        const refStr = type === 1 ? 'userMessagesNotTranslated' : 'userMessages'
        messagesToSave.push(admin.database().ref(`${refStr}/${uid}/${messageId}`).set(message.data))
      }
    });
...
exports.textToSpeech = functions.https.onCall(async(data,context) =>{
  ...
  try {
    ...
    const destLang = data.lang
    const gender = data.gender || "FEMALE" 
    if(!text || !messageId || !destLang){
      throw Error("Missing params.")
    }
    

    const cacheTts = await admin.database().ref(`tts/${messageId}_${destLang}_${gender}`).once('value')
    if(cacheTts.exists()){
      return cacheTts.val()
    }
    const workingDir = path.join(os.tmpdir(), 'tts'); 
    if(!fs.existsSync(workingDir)){
      fs.mkdirSync(workingDir);
    }
    const fileName = `${messageId}_${destLang}_${gender}.mp3`
    const filePath = path.join(workingDir, fileName);

    await convertTextToSpeech(text, destLang, filePath, gender)
    const bucketName = "gs://app-mobile-translation-chat.appspot.com"
    const destFilePath = `tts/${fileName}`
    await storage
      .bucket(bucketName).upload(filePath, { destination: destFilePath })
    fs.unlinkSync(filePath)

    await admin.database().ref(`tts/${messageId}_${destLang}_${gender}`).set(destFilePath)

      // await storage
      // .bucket(bucketName).file().makePublic()
...
function getLangWithNoAccent(lang){
  if(lang.includes("-")){
    return lang.split("-")[0]
  }
  return lang
